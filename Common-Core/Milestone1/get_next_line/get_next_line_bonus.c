...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/08/26 18:45:48 by jgilaber          #+#    #+#             */
/*   Updated: 2026/08/26 18:51:19 by jgilaber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (ft_strchr(stash[fd], LINE_DELIM))
	{
		readed_line = ft_update_stash(&stash[fd]);
		if (!readed_line)
			return (free(stash[fd]), stash[fd] = NULL, NULL);
		return (readed_line);
	}
	readed_line = ft_read_line(fd, &stash[fd]);
	if (!readed_line)
		return (free(stash[fd]), stash[fd] = NULL, NULL);
	return (readed_line);
}
